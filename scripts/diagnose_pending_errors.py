...
def list_pending():
    ...
    memory = load_memory()
    pending = [e for e in memory if e.get("solution") in ["NEEDS_DIAGNOSIS", "Pending Diagnosis", ""]]
    ...
    print("AI AGENT: For each error below, analyze the error snippet and provide a solution.")
    print("Run: python scripts/diagnose_pending_errors.py solve <fingerprint> \"<solution>\" \"<root_cause>\"\n")
    print("-" * 80)
