...
async def test_autonomy_upgrade():
    ...
    bad_tick = Tick(symbol=symbol, bid=0, ask=0, last=0, ts=str(int(time.time() * 1000)))
    good_tick = Tick(symbol=symbol, bid=50000, ask=50001, last=50000.5, ts=str(int(time.time() * 1000)))
    ...
    assert runner.latest_tick.last == 50000.5, f"Bad tick was not ignored! Last price: {runner.latest_tick.last}"
    ...
    assert runner.broker.close_all.called, "Broker.close_all was NOT called during shutdown"
