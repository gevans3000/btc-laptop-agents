...
def monte_carlo_simulation(
    trades_pnl, starting_balance=10000.0, iterations=1000, bootstrap=True
):
    if not trades_pnl:
        return None
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Monte Carlo Simulation for Trading Strategies"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="bitunix",
        help="Data source for backtest (mock/bitunix)",
    )
    parser.add_argument(
        "--limit", type=int, default=1000, help="Number of bars for backtest"
    )
    parser.add_argument(
        "--iterations", type=int, default=1000, help="Number of MC iterations"
    )
    parser.add_argument("--starting-balance", type=float, default=10000.0)
    parser.add_argument(
        "--no-bootstrap",
        action="store_true",
        help="Use shuffle instead of sampling with replacement",
    )
    ...
    print(
        f"Loaded {len(pnls)} trades. Starting {args.iterations} Iterations ({mode_str})..."
    )

    results = monte_carlo_simulation(
        pnls, args.starting_balance, args.iterations, bootstrap=bootstrap
    )
