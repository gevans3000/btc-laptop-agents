...
def main():
    ...
    if not secret_key:
        logger.error(
            "MISSING CREDENTIALS! BITUNIX_API_SECRET or BITUNIX_SECRET_KEY must be set."
        )
    ...
    try:
        ...
        for i, pos in enumerate(positions):
            logger.info(
                f"      Pos {i + 1}: {pos.get('symbol')} {pos.get('qty')} @ {pos.get('entryPrice')}"
            )
        ...
        for i, o in enumerate(open_orders):
            logger.info(
                f"      Order {i + 1}: {o.get('symbol')} {o.get('side')} {o.get('qty')} @ {o.get('price')}"
            )
