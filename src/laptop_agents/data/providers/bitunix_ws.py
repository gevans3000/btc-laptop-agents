...
import websockets.client as ws_v2_client
...
class BitunixWSProvider:
            ...
            try:
                ...
                if sub_path.exists():
                    with open(sub_path, "r") as f:
                        ...
                        logger.info(
                            f"Loaded {len(self.subscriptions)} subscriptions from cache"
                        )
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                resp = await client.get(
                    f"https://fapi.bitunix.com/api/v1/futures/market/tickers?symbols={DEFAULT_SYMBOL}"
                )
                ...
                if server_date:
                    ...
                    logger.info(
                        f"Synchronized with Bitunix. Time offset: {self.time_offset:.2f}ms"
                    )
        except Exception as e:
            logger.warning(f"Failed to sync time via REST: {e}")

        logger.info(f"Connecting to Bitunix WS (Kline): {self.URL}")
        self.ws_kline = await ws_v2_client.connect(self.URL)
        logger.info(f"Connecting to Bitunix WS (Ticker): {self.URL}")
        self.ws_ticker = await ws_v2_client.connect(self.URL)
        self._running = True
        self.connected = True
    ...
    async def subscribe_kline(self, interval: str = "1m"):
        ...
        msg = {"op": "subscribe", "args": [f"kline.{interval}.{self.symbol}"]}
    ...
    async def subscribe_ticker(self):
        ...
        msg = {"op": "subscribe", "args": [f"ticker.{self.symbol}"]}
    ...
    async def _resubscribe(self):
        ...
        if (
            not self.ws_kline
            or not self.ws_ticker
            or not self.subscriptions
                ...
                if "kline" in topic:
                    ...
                    for item in items:
                        ...
                        try:
                            ...
                            if (
                                self.last_kline_ts
                                and new_ts > self.last_kline_ts + interval_sec
                            ):
                                logger.warning(
                                    f"Gap detected! {new_ts - self.last_kline_ts}s missing. Fetching from REST."
                                )
                                # Start gap fill task
                                asyncio.create_task(
                                    self.fetch_and_inject_gap(
                            ...
                            try:
                                if self.queue.full():
                                    ...
                                    logger.warning(
                                        "QUEUE_OVERFLOW: Dropped oldest market data item (Candle)"
                                    )
                            ...
                            if self.last_message_time % 60 < 2:  # Reduce spam
                                logger.info(
                                    f"WS Candle Update: {candle.ts} | {candle.close}"
                                )
                        except (ValidationError, TypeError, KeyError, ValueError) as e:
                            logger.error(
                                f"Failed to parse candle data: {e} | Data: {item}"
                            )
                    ...
                    for item in items:
                        ...
                        try:
                            # Use Pydantic for validation
                            validated_ticker = self.TickerMessage(**item)
                            tick = Tick(
                                symbol=self.symbol,
                                bid=validated_ticker.bidOnePrice or 0.0,
                                ask=validated_ticker.askOnePrice or 0.0,
                            ...
                            try:
                                if self.queue.full():
                                    self.queue.get_nowait()
                                    logger.warning(
                                        "QUEUE_OVERFLOW: Dropped oldest market data item (Tick)"
                                    )
                            ...
                            logger.error(
                                f"Failed to parse ticker data: {e} | Data: {item}"
                            )
    ...
    async def funding_rate(self) -> float:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                await exchange_rate_limiter.wait()  # Use shared limiter
                resp = await client.get(
                    f"https://fapi.bitunix.com/api/v1/futures/market/funding_rate?symbol={self.symbol}"
                )
                data = resp.json()
                if data.get("code") == 0:
    ...
    async def fetch_and_inject_gap(self, start_ts: int, end_ts: int):
        ...
        try:
            ...
            candles = await asyncio.to_thread(
                load_bitunix_candles, self.symbol, self.interval, limit=10
            )
            injected_count = 0
            for c in candles:
                try:
            ...
            if injected_count > 0:
                ...
                logger.warning(
                    f"Gap fill: Found 0 candles to inject for range {start_ts} to {end_ts}"
                )
        except Exception as e:
    ...
    async def _heartbeat_check(self):
        ...
        while self._running:
            ...
            if self.last_message_time > 0 and (
                time.time() - self.last_message_time > self.heartbeat_timeout_sec
            ):
                logger.warning(
                    f"No WS message for {self.heartbeat_timeout_sec}s, forcing reconnect"
                )
                self._running = False
                if self.ws_kline:
                    try:
    ...
    @retry(
        ...
        retry=retry_if_exception_type(
            (ConnectionClosed, ConnectionError, asyncio.TimeoutError, socket.gaierror)
        ),
    ...
    async def listen(self) -> AsyncGenerator[Union[Candle, Tick], None]:
        ...
        while True:
            try:
                ...
                kline_handler = asyncio.create_task(
                    self._handle_messages(self.ws_kline, "Kline")
                )
                ticker_handler = asyncio.create_task(
                    self._handle_messages(self.ws_ticker, "Ticker")
                )
                ...
                await asyncio.gather(
                    kline_handler, ticker_handler, return_exceptions=True
                )
