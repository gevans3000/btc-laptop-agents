...
def load_mock_candles(n: int = 200) -> List[Candle]:
    ...
    for i in range(n):
        ...
        candles.append(
            Candle(
                ts=ts_obj.isoformat(), open=o, high=h, low=low_val, close=c, volume=1.0
            )
        )
    return candles
...
def _get_bitunix_provider_class():
    ...
    if not provider_cls:
        raise RuntimeError(
            "No Bitunix provider class with .klines() found in laptop_agents.data.providers.bitunix_futures"
        )
...
def load_bitunix_candles(symbol: str, interval: str, limit: int) -> List[Candle]:
    ...
    for c in rows:
        ts = (
            getattr(c, "ts", None)
            or getattr(c, "time", None)
            or getattr(c, "timestamp", None)
            or ""
        )
        o = float(getattr(c, "open"))
        h = float(getattr(c, "high"))
        low_val = float(getattr(c, "low"))
        cl = float(getattr(c, "close"))
        v = float(getattr(c, "volume", 0.0) or 0.0)
        out.append(Candle(ts=str(ts), open=o, high=h, low=low_val, close=cl, volume=v))

    try:
        from laptop_agents.data.providers.bitunix_ws import get_ws_client

        ws = get_ws_client(symbol)
        latest = ws.get_latest_candle()
        if latest:
            if not out:
                out = [latest]
            else:
