...
def set_context(
    latest_dir: Path, append_event_fn: Callable[[Dict[str, Any]], None]
) -> None:
...
def parse_grid(grid_str: str, max_candidates: int = 200) -> list[dict[str, Any]]:
    ...
    if total_combos > max_candidates:
        ...
        append_event(
            {
                "event": "ValidationCandidateCount",
                "original": total_combos,
                "reduced": actual_combos,
                "max_allowed": max_candidates,
            }
        )
    ...
    for i in range(len(candles)):
        ...
        if fast_sma_val is None or slow_sma_val is None:
            equity_history.append(
                {"ts": current_candle.ts, "equity": float(realized_equity)}
            )
            continue
        ...
        if position is None:
            if signal == "BUY":
                qty, sp, tp = calculate_position_size(
                    realized_equity,
                    current_close,
                    risk_pct,
                    stop_bps,
            ...
            if exit_reason:
                ...
                pnl = (
                    (exit_slipped - entry_price) * position_quantity
                    if position == "LONG"
                    else (entry_price - exit_slipped) * position_quantity
                )
                ...
                total_fees += fees
                position = None
            elif (position == "LONG" and signal == "SELL") or (
                position == "SHORT" and signal == "BUY"
            ):
                # Crossover reversal
...
def run_backtest_bar_mode(
    candles: List[Candle], starting_balance: float, fees_bps: float, slip_bps: float
) -> Dict[str, Any]:
...
def run_validation(
    ...
    for k in range(validate_splits):
        ...
        test_res = run_backtest_on_segment(
            ...
            slip_bps,
            risk_pct,
            best_params["fast_sma"],
            best_params["slow_sma"],
            best_params["stop_bps"],
            best_params["tp_r"],
            max_leverage,
            intrabar_mode,
        )
        folds.append(
            {"fold_index": k, "best_params": best_params, "test_result": test_res}
        )
    ...
    leaderboard = sorted(
        all_train_results, key=lambda x: x.get("objective", -9999), reverse=True
    )[:10]
