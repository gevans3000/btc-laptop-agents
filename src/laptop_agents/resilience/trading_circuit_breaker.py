...
class TradingCircuitBreaker:
    ...
    def update_equity(self, equity: float, trade_pnl: Optional[float] = None) -> None:
        ...
        if self._starting_equity > 0:
            drawdown_pct = ((self._starting_equity - equity) / self._starting_equity) * 100
            if drawdown_pct >= self.max_daily_drawdown_pct:
                self._trip("max_daily_drawdown", f"Daily drawdown {drawdown_pct:.2f}% >= {self.max_daily_drawdown_pct}%")
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "drawdown_pct": ((self._starting_equity - self._current_equity) / self._starting_equity * 100) if self._starting_equity > 0 else 0,
        }
