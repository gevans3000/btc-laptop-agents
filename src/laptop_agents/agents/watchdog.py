...
# Config
HEARTBEAT_PATH = REPO_ROOT / ".workspace/logs/heartbeat.json"
# Fallback to local logs if not running in repo structure
if not HEARTBEAT_PATH.parent.exists():
    HEARTBEAT_PATH = Path("logs/heartbeat.json")

MAX_AGE_SEC = 30.0  # Increased tolerance
CHECK_INTERVAL = 2.0
    ...
    while True:
        try:
            if not HEARTBEAT_PATH.exists():
                # Don't spam if file doesn't exist yet (startup)
                pass
                ...
                with open(HEARTBEAT_PATH, "r") as f:
                    try:
                        data = json.load(f)
                        last_ts = (
                            data.get("last_updated_ts") or data.get("unix_ts") or mtime
                        )
                ...
                if effective_age > MAX_AGE_SEC:
                    consecutive_stale += 1
                    print(
                        f"[{time.strftime('%H:%M:%S')}] ALERT: Heartbeat STALE ({effective_age:.1f}s)"
                    )
                    if consecutive_stale >= 3:
                        print(
                            f"[{time.strftime('%H:%M:%S')}] CRITICAL: Process likely FROZEN."
                        )
                else:
                    if consecutive_stale > 0:
                        print(
                            f"[{time.strftime('%H:%M:%S')}] RECOVERED: Heartbeat is back (Age: {effective_age:.1f}s)"
                        )
                    consecutive_stale = 0
