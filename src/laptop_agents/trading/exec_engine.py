...
def run_live_paper_trading(
    ...
    if actual_max_leverage != max_leverage:
        _append_event({"event": "HardLimitLeverageCapped", "requested": max_leverage, "capped": actual_max_leverage})
        max_leverage = actual_max_leverage
    
    actual_tp_r = max(tp_r, getattr(hard_limits, "MIN_RR_RATIO", 1.0))
    if actual_tp_r != tp_r:
        _append_event({"event": "HardLimitRRCapped", "requested": tp_r, "capped": actual_tp_r})
        tp_r = actual_tp_r
    ...
    if state.get("realized_pnl", 0.0) <= -max_daily_loss:
        _append_event({"event": "HardLimitDailyLossReached", "pnl": state["realized_pnl"], "limit": max_daily_loss})
    ...
    for candle in new_candles:
        ...
        if position is None:
            ...
            if signal == "BUY":
                ...
                if qty is None:
                    _append_event({"event": "RiskSizingSkipped", "reason": "invalid stop distance or qty"})
                    continue
                ...
                if qty * current_close > max_notional_usd:
                    ...
                    _append_event({"event": "HardLimitPositionCapped", "original_qty": qty, "capped_qty": new_qty, "limit": max_notional_usd})
                    qty = new_qty

                entry_price_slipped = apply_slippage(current_close, True, True, slip_bps)
                ...
                _append_event({"event": "PositionOpened", "side": "LONG", "ts": candle.ts,
                             "price": entry_price_slipped, "quantity": qty, "stop": stop_price, "tp": tp_price})
            elif signal == "SELL":
                qty, stop_price, tp_price = calculate_position_size(
                    equity=state["equity"],
                    entry_price=current_close,
                ...
                entry_price_slipped = apply_slippage(current_close, True, False, slip_bps)
                entry_fees = calculate_fees(entry_price_slipped * qty, fees_bps)
                ...
                _append_event({"event": "PositionOpened", "side": "SHORT", "ts": candle.ts,
                             "price": entry_price_slipped, "quantity": qty, "stop": stop_price, "tp": tp_price})
        else:
            ...
            if exit_reason is not None:
                exit_price_slipped = apply_slippage(exit_price, False, position["side"] == "LONG", slip_bps)
                ...
                exit_fees = calculate_fees(exit_price_slipped * position["quantity"], fees_bps)
                ...
                _append_event({"event": "PositionClosed", "side": position["side"], "ts": candle.ts,
                ...
                exit_price_slipped = apply_slippage(current_close, False, position["side"] == "LONG", slip_bps)
