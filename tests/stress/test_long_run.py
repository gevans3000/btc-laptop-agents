...
async def test_memory_leak_long_run():
    ...
    try:
        ...
        pass # Runner might exit when provider finishes if we handled that loop right (we didn't, it loops forever in real runner)
    ...
    print(f"Start Mem: {start_mem:.2f}MB, End Mem: {end_mem:.2f}MB, Growth: {growth:.2f}MB")
