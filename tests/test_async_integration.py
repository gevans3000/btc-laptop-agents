import pytest
import asyncio
from laptop_agents.session.async_session import run_async_session, AsyncRunner
...
class MockProvider:
    ...
    async def listen(self):
        ...
        for i in range(5):
            yield Tick(symbol=self.symbol, bid=50000.0, ask=50001.0, last=50000.5, ts=str(1700000000 + i))
            await asyncio.sleep(0.01)
        
        for i in range(5):
            yield Candle(ts=str(1700000000 + i * 60), open=50000.0, high=50100.0, low=49900.0, close=50050.0, volume=1.0)
            await asyncio.sleep(0.01)
