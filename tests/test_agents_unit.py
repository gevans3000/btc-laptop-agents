...
def base_candles():
    return [
        Candle(ts="2024-01-01T00:00:00Z", open=40000, high=40100, low=39900, close=40050, volume=10),
        Candle(ts="2024-01-01T00:01:00Z", open=40050, high=40200, low=40000, close=40150, volume=12),
    ]
...
def test_market_intake_run(base_state):
    ...
    for i in range(55):
        base_state.candles.append(Candle(ts=f"T{i}", open=40000, high=40100, low=39900, close=40050, volume=10))
...
def test_derivatives_flows_run(base_state):
    ...
    gates = {"extreme_funding_8h": 0.001, "no_trade_funding_8h": 0.0005, "half_size_funding_8h": 0.0003}
...
def test_setup_signal_run(base_state):
    base_state.market_context = {"price": 40150, "trend": "UP", "ema20": 40000, "atr": 200}
    cfg = {
        "pullback_ribbon": {"enabled": True, "entry_band_pct": 0.001, "stop_atr_mult": 2.0, "tp_r_mult": 3.0},
