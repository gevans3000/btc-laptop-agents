...
def test_partial_fill_creates_working_order():
    ...
    assert broker.working_orders[0]["qty"] == pytest.approx(0.99)
...
def test_working_order_fills_on_next_candle():
    ...
    candle1 = Candle(ts="2024-01-01T00:00:00", open=100.0, high=101.0, low=99.0, close=100.0, volume=0.1)
    order = {
        "go": True, "side": "LONG", "entry_type": "market", "entry": 100.0, "qty": 1.0,
        "sl": 90.0, "tp": 120.0, "equity": 10000.0, "client_order_id": "test_wo_001"
    }
    ...
    candle2 = Candle(ts="2024-01-01T00:01:00", open=100.0, high=101.0, low=99.0, close=100.0, volume=100.0)
    events = broker.on_candle(candle2, None)
    
    import pytest
    assert len(events["fills"]) == 1
    assert events["fills"][0]["qty"] == pytest.approx(0.99)
